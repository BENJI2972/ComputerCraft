	...
	function alarm()
		...
		if rs.getInput(buttonside) then
			alarmstatut = not(alarmstatut)
			sleep(1)
		end
		
		if (alarmstatut == true and   (reactorStatut == true and reactorOutput == 0)   )  then
	...
	while true do
		...
		parallel.waitForAll(alarm,manage,monitoring,statut)
